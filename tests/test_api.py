...
def test_fastapi_endpoints():
    ...
    auth_headers = {"Authorization": f"Bearer {settings.app_auth_token or 'kai5108_secret_passcode_2026'}"}

    # 1. Test Static & Root
    resp_root = client.get("/")
    assert resp_root.status_code == 200
    assert "КАИ 5108" in resp_root.text or "Student OS" in resp_root.text
    ...
    # 3. Test /api/stats (Unauthenticated should be 401)
    resp_unauth = client.get("/api/stats")
    assert resp_unauth.status_code == 401

    # Authenticated stats
    resp_stats = client.get("/api/stats", headers=auth_headers)
    ...
    # 4. Test /api/subjects
    resp_subjs = client.get("/api/subjects", headers=auth_headers)
    ...
    # 5. Test /api/tasks
    resp_tasks = client.get("/api/tasks?status=all", headers=auth_headers)
    ...
    resp_toggle = client.post(f"/api/tasks/{t_id}/toggle", headers=auth_headers)
    ...
    resp_toggle_back = client.post(f"/api/tasks/{t_id}/toggle", headers=auth_headers)
    assert resp_toggle_back.status_code == 200
    assert resp_toggle_back.json()["status"] == orig_status
    print("Task toggle test OK!")

    # 7. Test /api/schedule
    resp_sched = client.get("/api/schedule?day=2&week=чет", headers=auth_headers)
    ...
    resp_week = client.get("/api/schedule/week?week=чет", headers=auth_headers)
